...
class sgwindow_slider extends WP_Widget {
	...
	*/
	function widget( $args, $instance ) {
		...
		if ( 1 == $instance['content_type'] ) {
			...
			$selectCat = (!empty($idcategoria = get_query_var('cat'))) ? get_query_var('cat'): $instance['category'];

			if( '0' != $selectCat ) {
				$args =  array(
					array(
						'taxonomy' => $tax,
						'terms'    => array( $selectCat ),
						'field'    => 'term_id',
						'operator' => 'IN',
					),
				);
					...
					<?php for ( $i = 0; $i < $instance['count']; $i++ ) : ?>
						<article class="sgwindow-slide">
							...
							<?php if ( '1' == $instance[ 'is_link_' . $i ] ) : ?>
								<a target="_blank" class="slide-link" href="<?php echo esc_url( $instance['link_' . $i ] ); ?>">
							<?php endif; ?>
									<div class="sgwindow-slide-content" <?php if ( '1' == $instance['is_background'] ) : ?>style="background-image: url(<?php echo esc_url( $instance['img_' . $i ] ); ?>); -webkit-background-size: <?php echo esc_attr( $instance['height'] ); ?>%; background-size: <?php echo esc_attr( $instance['height'] ); ?>%;" <?php endif; ?>>
	...
	*/
	function update( $new_instance, $old_instance ) {
		...
		$instance['margin'] = $instance['margin'] > 50 ? 20 : $instance['margin'];
